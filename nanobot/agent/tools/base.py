...
class ToolRegistrationMetadata(TypedDict):
    """Standard metadata exposed by every registered tool."""

    name: str
    description: str
    parameters: dict[str, Any]
    config_key: str
    plugin_discoverable: bool
    scopes: tuple[str, ...]
    read_only: bool
...
class Tool(ABC):
    ...
    def validate_registration(self) -> list[str]:
        """Validate static tool metadata before registration.

        This is intentionally narrower than full JSON Schema validation: it checks
    ...
    def registration_metadata(self) -> ToolRegistrationMetadata:
        ...
        return {
            ...
            "plugin_discoverable": self._plugin_discoverable,
            "scopes": tuple(sorted(self._scopes)),
