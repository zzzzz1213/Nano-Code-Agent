...
BUILTIN_COMMAND_SPECS: tuple[BuiltinCommandSpec, ...] = (
    ...
    BuiltinCommandSpec(
        "/resume-safe-tools",
        "Resume safe tools",
        "Confirm and resume read-only recovery candidates from the latest checkpoint.",
        "refresh-cw",
    ),
    BuiltinCommandSpec(
        "/restart",
...
async def cmd_resume_safe_tools(ctx: CommandContext) -> OutboundMessage:
    """Resume the safe pending tools captured in the latest runtime checkpoint."""
    loop = ctx.loop
    msg = ctx.msg
    ...
    if checkpoint is None:
        return OutboundMessage(
            channel=msg.channel,
            chat_id=msg.chat_id,
            content="No recoverable checkpoint with safe tools was found.",
            metadata=dict(msg.metadata or {}),
        )

    await loop._publish_recovered_runtime_checkpoint(msg, session, turn_id=ctx.key)

    resumed = len(checkpoint.get("recovered_executed_tool_call_ids") or [])
    skipped = len(checkpoint.get("recovered_skipped_tool_call_ids") or [])
    pending = len(checkpoint.get("pending_tool_call_ids") or [])
    requires_user = len(checkpoint.get("recovered_requires_user_tool_call_ids") or [])
    review_required = len(checkpoint.get("review_required_tool_call_ids") or [])
    needs_input = len(checkpoint.get("needs_input_tool_call_ids") or [])
    blocked = len(checkpoint.get("blocked_tool_call_ids") or [])
    if resumed > 0:
        content = f"Resumed {resumed} safe tool(s)."
    else:
        content = "No safe tools were available to resume."
    if pending > 0:
        content += f" {pending} tool(s) remain pending."
    if review_required > 0:
        content += f" {review_required} tool(s) need review before retry."
    if needs_input > 0:
        content += f" {needs_input} tool(s) need user input."
    if blocked > 0:
        content += f" {blocked} tool(s) are blocked by safety policy."
    elif requires_user > 0:
        content += f" {requires_user} tool(s) still need review."
    if skipped > 0:
        content += f" {skipped} candidate(s) were skipped."
    return OutboundMessage(
        channel=msg.channel,
        chat_id=msg.chat_id,
        content=content,
...
def register_builtin_commands(router: CommandRouter) -> None:
    ...
    router.exact("/resume-safe-tools", cmd_resume_safe_tools)
